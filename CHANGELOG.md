...
### Dashboard
- 网站测速标签状态徽章右对齐、固定宽度，解决未测试/延迟/失败文字错位。
- 新增国内外出口 IP 卡（直连国内出口 + 代理国际出口，多源回退，一键刷新）。
- 流量图补图例、累计流量与触控 Tooltip；favicon 双源回退。

### Nodes & Routing
- 节点国旗自动解析、多选全选/反选、搜索一键清空、桌面右键菜单与双击启动。
- 路由规则上移/下移（带序号徽章），DNS 并入分流页第三标签，移动端导航精简为 5 项。
- 全局状态栏移至根布局，所有页面常驻在线状态与实时速率。

### Settings & i18n
- 语言切换、主题强调色（4 种）、订阅自动更新（6/12/24h）、关于页（含仓库链接）。
- 协议表单 57 个字段标签、校验报错、DNS 策略、托盘菜单/提示全面国际化（中英 270+ 词条对齐）。
- DNS 输入失焦自动保存 + 格式校验；按钮样式统一为 FilledButton.tonal。

### Brand
- 新霓虹猫 Logo：原生启动图标（Android/Windows/macOS，经 CI 自动生成）、导航栏与关于页同步替换。

### Known Limitations
- 更新包无发布者签名（仅 SHA256 校验）；连接明细、规则测试器、测速站自定义、二维码在路线图中。
...
### New Features & UX
- **仪表盘节点下拉切换**：底部弹窗改为下拉框；选择与运行状态分离——停止时切换只换选中节点（保持停止），运行时切换热切换到新节点。
- **实时速率修复**：core 配置补上 `experimental.v2ray_api.stats`（此前计数器从未创建，上下行恒为 0），仪表盘速率实时显示；布局压缩（hero 卡、单行速率条、图表 100px）。
- **网站测速大标签**：站点 chip 放大，带 favicon 图标（失败回退内置图标）、域名副行与延迟徽章。
- **备份导出到文件**：修复导出按钮错标"导出日志"问题；经系统保存对话框直接写 JSON 文件，无对话框环境回退到内部快照。
- **从文件导入配置**：导入对话框新增文件选择（保留手动粘贴兜底）。
- **移除旧版 C++ 配置迁移**：需求取消，UI 入口、逻辑与 `migration.dart` 一并删除。

### Engineering
- 新增 `file_picker` 依赖（v12 API：`FilePicker.saveFile/pickFile`）。
- sing-box 保持 v1.13.19：1.14 需 Go 1.25 且废弃我方在用的 DNS/规则集字段，单独立项（见 DECISIONS D9）。
